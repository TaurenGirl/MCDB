...
impl Collection {
    pub fn sort_by_name(mut self) {
        self.contents.sort_by(|a,b| a.name.cmp(&b.name));
    }

    pub fn sort_by_aspect(mut self) {
        self.contents.sort_by(|a, b| a.aspect.cmp(&b.aspect));
    }

    pub fn sort_by_owner(mut self) {
        self.contents.sort_by(|a, b| a.owner.cmp(&b.owner));
    }
}
...
pub fn populate(start_tag: &str, end_tag: &str, file_path: String) -> Collection {
    ...
    println!("Attempting to search in {}", file_path);
    let contents = read_to_string(file_path).expect("Should have been able to read file.");
    let starts: Vec<_> = contents.match_indices(start_tag).collect();
    let ends: Vec<_> = contents.match_indices(end_tag).collect();
    for (i, el) in starts.clone().iter_mut().enumerate() {
        let start_actual = el.0 + el.1.len();
        let data_tmp = &contents[start_actual..ends[i].0];
        let card = Card {
